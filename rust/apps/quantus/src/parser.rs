...
#[derive(DeriveDecode, Debug, PartialEq)]
...
/// bytes left over. Everything the device signs is either displayed or validated.
#[derive(Debug)]
...
pub fn bytes_to_ss58(bytes: &[u8; 32]) -> String {
    ...
    crate::ss58::encode(bytes, SS58_PREFIX)
}
...
mod tests {
    ...
    // Call portions of real signed payloads (extensions stripped); the original vectors were
    // captured on a retired devnet whose genesis hash this firmware no longer accepts.
    ...
    // Real signed payload from the retired devnet (genesis 826beefb…), kept verbatim to prove
    // the parser walks all the way to the genesis hash and rejects unknown networks.
    const OLD_NETWORK_PAYLOAD: &str =
    ...
    fn test_reject_oversized_payload() {
        let payload = alloc::vec![0u8; MAX_PAYLOAD_BYTES + 1];
        let err = parse_payload(&payload).unwrap_err();
    ...
    fn propose_wrapping(inner: &[u8]) -> Vec<u8> {
        let mut call = alloc::vec![0x13, 0x01];
        call.extend_from_slice(&[0x99; 32]);
        call.extend(Compact(inner.len() as u32).encode());
        call.extend_from_slice(inner);
